...
const LoginModal = (props) => {
  ...
  const { confirmUser, forgotPassword, resetCodeSent, signingIn, signingUp } = user
    ...
    const handleFBClick = () => {
      ...
      window.location.href=`https://crowdsourcedscores.auth.us-west-2.amazoncognito.com/oauth2/authorize?identity_provider=Facebook&redirect_uri=${window.location.hostname === 'localhost' ? `http://` : `https://`}${window.location.host}/callback&response_type=CODE&client_id=2n15lhk845sucm0k4fejjqcbev&state=${state}&scope=aws.cognito.signin.user.admin+email+openid+phone+profile`
    }
